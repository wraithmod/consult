...
## Completed Tasks
- TASK-1: Ollama streaming implemented in `src/summarise.py` with NDJSON token assembly, stderr token streaming, 10 s chunk timeout, and 120 s wall-clock guard.
- TASK-2: faster-whisper backend added in `src/transcribe.py` (first preference), with device handling, compute type selection, and `--device` CLI support.
- TASK-3: `src/pipeline.py` refactored to default in-process execution with sequential step functions and `--subprocess` legacy fallback.
- TASK-4: tests updated for streaming summarise path, faster-whisper mocking/device behaviour, and in-process pipeline path; verified with `PYTHONPATH=. pytest -q tests` (53 passed).
